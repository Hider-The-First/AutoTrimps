...
//            if (!game.portal.Overkill.locked && game.stats.cellsOverkilled.value)
    ...
    if (shouldDoMaps || doVoids || needPrestige) {
        ...
        if (selectedMap == "world") {
            ...
            if (needPrestige)
                if (game.global.world == game.global.mapsOwnedArray[highestMap].level && 26 == game.global.mapsOwnedArray[highestMap].size && 182 < game.global.mapsOwnedArray[highestMap].loot && game.global.mapsOwnedArray[highestMap].location == "Plentiful")
                    selectedMap = game.global.mapsOwnedArray[highestMap].id;
                else
                    selectedMap = "create";
            //if needFarmSpire x minutes is true, switch over from wood maps to metal maps.    
            else if (needFarmSpire)
                if (26 == game.global.mapsOwnedArray[highestMap].size && 182 < game.global.mapsOwnedArray[highestMap].loot && game.global.mapsOwnedArray[highestMap].location == "Plentiful") //Mountain
                    selectedMap = game.global.mapsOwnedArray[highestMap].id;
                else
                    selectedMap = "create";
            //if shouldFarm is true, use a siphonology adjusted map, as long as we aren't trying to prestige                                
            else if (siphonMap != -1 && 26 == game.global.mapsOwnedArray[highestMap].size && 182 < game.global.mapsOwnedArray[highestMap].loot && game.global.mapsOwnedArray[highestMap].location == "Plentiful")
