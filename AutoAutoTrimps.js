...
function getNiceThingsDone() {
	if (!game.global.preMapsActive && !game.global.mapsActive && game.global.gridArray.length === 0) {
		return;;
	}else if (game.global.mapsActive && game.global.mapGridArray.length === 0) {
		return;;
	}
	if (!game.global.preMapsActive && !game.global.mapsActive && !game.global.spireActive && (new Date().getTime() - game.global.zoneStarted) > 150) {
	...
	} else if (game.global.mapsActive && getCurrentMapObject().location == "Void" && (new Date().getTime() - game.global.mapStarted) > 150) {
	...
	var cells = document.getElementById("mapGrid").getElementsByClassName("battleCell cellColorBeaten"); var oldstyle = cells[0].getAttribute('style'); for (var i=0; i < cells.length; i++) cells[i].setAttribute('style', oldstyle + '; background-color: rgba(0,0,0,0.3);');
	} else if (game.global.world == 200 && game.global.spireActive && (new Date().getTime() - game.global.zoneStarted) > 150) {
