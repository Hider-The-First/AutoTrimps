...
function getNiceThingsDone() {
	...
	document.getElementById("trimps").insertAdjacentHTML('afterend', '<div id="pic"><img style="max-height: 9000vw; vertical-align; background;" src="http://randomwallpapers.net/dark-stairs-photography-1920x1200-wallpaper75304.jpg"></div><div');
...
function autoStance() {
    ...
    if ((!game.global.mapsActive && !game.global.preMapsActive && ((hiderwindow > 90 && game.global.lastClearedCell == 98) || game.global.lastClearedCell < 98)) && ((!getCurrentEnemy(1).corrupted && hiderwindow > 90) ||
    ...
    if (shouldDoMaps || doVoids || needPrestige) {
        ...
        if (selectedMap == "world") {
            ...
            //if needFarmSpire x minutes is true, switch over from wood maps to metal maps.    
                ...
                if (game.global.mapsOwnedArray[highestMap].location == 'Mountain') //Gardens
                    selectedMap = game.global.mapsOwnedArray[highestMap].id;
                else
                    selectedMap = "create";
    ...
    //clicks the maps button, once or twice (inside the world):
        ...
        if (selectedMap == "world") {
            ...
            if (game.global.world == 200 && game.global.spireActive) {
                ...
                biomeAdvMapsSelect.value = "Mountain";    //Gardens
                updateMapCost();                
                ...
                biomeAdvMapsSelect.value = "Sea"; // Gardens
                updateMapCost();
            } else {
                sizeAdvMapsRange.value = 9;
                adjustMap('size', 9);
...
function useScryerStance() {
    ...
    if (useoverkill && game.portal.Overkill.level > 0) {
        ...
        if (game.global.preMapsActive || (!game.global.preMapsActive && game.global.mapsActive && getCurrentMapObject().location == "Void") || HDratio > 7 || (game.global.spireActive && game.global.lastClearedCell > 77) || game.global.gridArray.length === 0 || game.global.highestLevelCleared < 180 || (hiderwindow < 90 && game.global.lastClearedCell == 98)) { autoStance(); return;
        }
        if (ovklHDratio > 0.6) {
            setFormation(4);
            return;
