...
workBtn pointer noselect.setAttribute("onmouseover", 'tooltip(\"OverKill Chance\", \"customText\", event, \"Get Dark Essence if over 300%.<br>Buy more Prestige if over 75%.<br>Save high level Void Maps if over 50%.<br>Farm Void Maps if under 30%.<br>Ignore high level Void Maps limits if over 9%.<br>Get +20% Map Bonus if in map and Breeding and if under 3%.<br>Use Dominance Formation in maps if under 3%.<br>Allow He/Hr Auto Portal with Void Maps if under 3%.<br>Get 200% Map Bonus if under 2.5%.<br>He/Hr Auto Portal right after Void Maps Settings if under 1%.\")');
workBtn pointer noselect.setAttribute("onmouseout", 'tooltip("hide")');
//setup talk window
